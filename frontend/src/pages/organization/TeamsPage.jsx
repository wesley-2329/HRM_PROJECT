...
const TeamsPage = ({ mode }) => {
  ...
  useEffect(() => {
    fetchData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, limit, statusFilter, deptFilter, sortBy, sortOrder]);
  ...
  const validateForm = () => {
    if (!name || !code || !parentDeptId) {
      showToast('Name, code, and department are required', 'warning');
      return false;
    }
    if (maxMembers <= 0) {
      showToast('Maximum members must be greater than 0', 'warning');
      return false;
    }
    return true;
  };
  ...
  const handleSave = async (e) => {
    ...
    const payload = {
      name, code, parentDeptId, teamLead, description, maxMembers, status
    };

    try {
      if (editingItem) {
        await api.put(`/org/teams/${editingItem._id}`, payload);
        showToast('Team details updated successfully', 'success');
      } else {
        await api.post('/org/teams', payload);
        showToast('Team setup registered successfully', 'success');
      }
      fetchData();
      setShowAddEditModal(false);
      resetForm();
    } catch (err) {
      showToast(err.response?.data?.message || 'Failed to save team setup', 'error');
    }
  };
  ...
  const openEdit = (item) => {
    ...
    setName(item.name);
    setCode(item.code);
    setParentDeptId(item.parentDeptId?._id || item.parentDeptId || '');
    setTeamLead(item.teamLead || '');
    setDescription(item.description || '');
    setMaxMembers(item.maxMembers || 10);
  ...
  const confirmDelete = async () => {
    try {
      await api.delete(`/org/teams/${deletingItem._id}`);
  ...
  const toggleStatus = async (item) => {
    ...
    try {
      await api.put(`/org/teams/${item._id}`, { status: newStatus });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const res = await api.post('/org/teams/bulk-delete', { ids: selectedIds });
  ...
  const handleBulkStatus = async (newStatus) => {
    try {
      const res = await api.put('/org/teams/bulk-status', { ids: selectedIds, status: newStatus });
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Team Name', 'Team Code', 'Department', 'Team Lead', 'Max Members', 'Current Members', 'Status'];
    const rows = data.map(item => {
      const dept = departments.find(x => x._id === item.parentDeptId || x._id === item.parentDeptId?._id);
      const lead = employees.find(x => x.id === item.teamLead);
      return [
        item.name, item.code, dept ? dept.name : 'N/A', lead ? lead.name : 'N/A',
        item.maxMembers, item.currentMembers || 0, item.status
      ];
    });
    ...
    link.setAttribute("download", "teams_setup.csv");
  ...
  const handleExportExcel = () => {
    if (data.length === 0) return showToast('No data to export', 'warning');
    let excelContent = '<table><tr><th>Team Name</th><th>Team Code</th><th>Department</th><th>Team Lead</th><th>Max Members</th><th>Status</th></tr>';
    data.forEach(item => {
      const dept = departments.find(x => x._id === item.parentDeptId || x._id === item.parentDeptId?._id);
      const lead = employees.find(x => x.id === item.teamLead);
      excelContent += `<tr><td>${item.name}</td><td>${item.code}</td><td>${dept ? dept.name : 'N/A'}</td><td>${lead ? lead.name : 'N/A'}</td><td>${item.maxMembers}</td><td>${item.status}</td></tr>`;
    });
    excelContent += '</table>';
    const blob = new Blob([excelContent], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "teams_setup.xls";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <div style={{ fontSize: '0.8rem', color: 'hsl(var(--text-secondary))', marginBottom: '12px' }}>
        <span>Directory</span> / <span style={{ fontWeight: 600, color: 'hsl(var(--primary))' }}>Team Setup</span>
      </div>

      <SubmoduleHeader 
        title="Team Setup" 
        description="Configure operational workgroups, map parent functional departments, select team leads, and manage group capacity boundaries."
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            <select className="form-control" value={deptFilter} onChange={e => { setDeptFilter(e.target.value); setPage(1); }} style={{ width: '160px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid hsl(var(--border))', color: 'hsl(var(--text-secondary))', fontSize: '0.85rem' }}>
                  ...
                  <th style={{ padding: '12px' }}>Max Members</th>
                  <th style={{ padding: '12px' }}>Current Members</th>
                ...
                {loading ? (
                  ...
                  [...Array(limit)].map((_, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid hsl(var(--border))' }}>
                      ...
                      <td style={{ padding: '12px' }}><div className="skeleton" style={{ height: '16px', width: '100px' }}></div></td>
                      <td style={{ padding: '12px' }}><div className="skeleton" style={{ height: '16px', width: '50px' }}></div></td>
                      <td style={{ padding: '12px' }}><div className="skeleton" style={{ height: '16px', width: '50px' }}></div></td>
                  ...
                  <tr>
                    <td colSpan={9} style={{ padding: '40px', textAlign: 'center' }}>
                      ...
                      <h4 style={{ margin: '0 0 6px 0', fontWeight: 600 }}>No Teams found</h4>
                      <p style={{ margin: 0, fontSize: '0.85rem', color: 'hsl(var(--text-secondary))' }}>
                        Try search modifications or click the register action above to create a new team.
                      </p>
                    </td>
                  </tr>
                ) : (
                  ...
                  data.map(item => (
                    <tr key={item._id} style={{ borderBottom: '1px solid hsl(var(--border))', fontSize: '0.9rem' }}>
                      ...
                      <td style={{ padding: '12px' }}>{getDeptName(item.parentDeptId)}</td>
                      <td style={{ padding: '12px' }}>{getEmpName(item.teamLead)}</td>
                      <td style={{ padding: '12px', fontWeight: 600 }}>{item.maxMembers}</td>
                      <td style={{ padding: '12px' }}>
                        <span className={`badge ${item.currentMembers >= item.maxMembers ? 'badge-danger' : 'badge-secondary'}`}>
          ...
          {!loading && data.length > 0 && (
            <div style={{
              ...
              <div style={{ fontSize: '0.85rem', color: 'hsl(var(--text-secondary))' }}>
                Showing {data.length} of {total} teams
              </div>
      ...
      {/* Add / Edit Modal */}
      <Modal isOpen={showAddEditModal} onClose={() => setShowAddEditModal(false)} title={editingItem ? 'Edit Team Details' : 'Register New Team'}>
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '14px', minWidth: '400px' }}>
          ...
          <div className="form-group">
            ...
            <select className="form-control" value={parentDeptId} onChange={e => setParentDeptId(e.target.value)} required>
          ...
          <div className="form-group">
            ...
            <select className="form-control" value={teamLead} onChange={e => setTeamLead(e.target.value)}>
          ...
          <div className="form-group">
            ...
            <input type="number" className="form-control" value={maxMembers} onChange={e => setMaxMembers(parseInt(e.target.value))} required min={1} />
          </div>
      ...
      {/* View Details Modal */}
      <Modal isOpen={showViewModal} onClose={() => setShowViewModal(false)} title="Team Profile Setup">
        {viewingItem && (
          <div style={{ minWidth: '400px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '10px 16px', fontSize: '0.9rem' }}>
              ...
              <div>{getDeptName(viewingItem.parentDeptId)}</div>

              <strong>Team Lead / Mgr:</strong>
              <div>{getEmpName(viewingItem.teamLead)}</div>

              <strong>Max Member Limit:</strong>
              <div>{viewingItem.maxMembers} members</div>

              <strong>Current Member Count:</strong>
              <div>{viewingItem.currentMembers || 0} members</div>
            ...
            <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'flex-end' }}>
              <button className="btn btn-secondary" onClick={() => setShowViewModal(false)}>Close Profile</button>
            </div>
          </div>
        )}
      </Modal>
        ...
        {deletingItem && (
          <div style={{ minWidth: '320px' }}>
            <p style={{ fontSize: '0.95rem', margin: '0 0 20px 0' }}>
              Are you sure you want to delete the team <strong>{deletingItem.name}</strong> ({deletingItem.code})? This will check capacity and perform a soft-delete.
            </p>
